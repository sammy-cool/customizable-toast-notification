...
import { createToast, setDefaultColors, setDefaultMessages } from '../../src/index.js';
...
describe('Toast Library Functions', () => {
  ...
  test('createToast creates toast element with full verification', () => {
    ...
    const toastElement = Array.from(document.querySelectorAll('div'))
      .find(div => div.id && div.id.includes('toast-') && !div.id.includes('container'));
  ...
  test('createToast error type (individual test)', () => {
    ...
    if (toastElement) {
      ...
      expect(() => createToast({ message: 'Error message', type: 'error' })).not.toThrow();
    }
  });
